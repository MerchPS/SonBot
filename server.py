#!/usr/bin/env python3
"""
🎵 BIMOLI SOUNDBOARD SERVER - Silent Mode
"""
...
def send_to_discord(message):
    """Kirim notifikasi ke Discord (silent)"""
    try:
...
if __name__ == '__main__':
    ...
    # Kirim IP ke Discord (SILENT)
    send_to_discord(f"🟢 **Soundboard Server Aktif!**\n📱 **URL HP:** http://{ip}:{PORT}\n🔊 Volume: {VOLUME_GLOBAL*100:.0f}%\n⏰ {__import__('datetime').datetime.now().strftime('%H:%M:%S')}")
